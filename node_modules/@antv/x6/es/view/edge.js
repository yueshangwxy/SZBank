...
export class EdgeView extends CellView {
    ...
    normalizeLabelMarkup(markup) {
        ...
        if (childNodes.length > 1 || childNodes[0].nodeName.toUpperCase() !== 'G') {
            ...
            vel = Dom.createVector(childNodes[0]);
        }
    ...
    renderVertexMarkers() {
        ...
        if (Markup.isStringMarkup(markup)) {
            ...
            this.cell.getVertices().forEach((vertex, index) => {
                $container.append(Dom.createVector(template(Object.assign({ index }, vertex))).node);
            });
        }
        return this;
    }
        ...
        if (data.currentTarget !== target) {
            ...
            if (data.currentView) {
                ...
                if (data.currentMagnet &&
                    this.graph.hook.validateConnection(...data.getValidateConnectionArgs(data.currentView, data.currentMagnet))) {
            ...
            if (view.container.getAttribute('magnet') !== 'false') {
                ...
                if (distance < radius && distance < minDistance) {
                    if (prevMagnet === view.container ||
                        graph.hook.validateConnection(...data.getValidateConnectionArgs(view, null))) {
                ...
                if (magnet.getAttribute('magnet') !== 'false') {
                    ...
                    if (distance < radius && distance < minDistance) {
                        if (prevMagnet === magnet ||
                            graph.hook.validateConnection(...data.getValidateConnectionArgs(view, magnet))) {
                            minDistance = distance;
                            data.closestView = view;
    ...
    highlightAvailableMagnets(data) {
        ...
        for (let i = 0, ii = cells.length; i < ii; i += 1) {
            ...
            const availableMagnets = magnets.filter((magnet) => graph.hook.validateConnection(...data.getValidateConnectionArgs(view, magnet)));
    ...
    stopArrowheadDragging(e, x, y) {
        ...
        FunctionExt.toDeferredBoolean(graph.hook.validateEdge(this.cell, data.terminalType, data.initialTerminal)).then((valid) => {
            if (valid) {
