...
export class Stencil extends View {
    constructor(options) {
        ...
        this.$container = this.$(this.container).addClass(this.prefixClassName(ClassNames.base));
    ...
    loadGroup(cells, groupName) {
        ...
        if (model) {
            const nodes = cells.map((cell) => cell instanceof Node ? cell : Node.create(cell));
        ...
        const found = Object.keys(this.graphs).reduce((memo, groupName) => {
            ...
            const items = graph.model.getNodes().filter((cell) => {
                ...
                if (typeof filter === 'function') {
                    ...
                    matched = this.isCellMatched(cell, keyworld, filter, keyworld.toLowerCase() !== keyworld);
                }
    ...
    isCellMatched(cell, keyworld, filters, ignoreCase) {
        if (keyworld && filters) {
...
(function (Stencil) {
    Stencil.defaultOptions = Object.assign({ stencilGraphWidth: 200, stencilGraphHeight: 800, title: 'Stencil', collapsable: false, layout(model, group) {
            const options = {
