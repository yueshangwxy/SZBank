 ...
 */
exports.boundary = function (line, view, magnet, options) {
    var node;
    var intersection;
    var anchor = line.end;
    ...
    if (typeof selector === 'string') {
        ...
        node = util_2.findShapeNode(magnet);
    }
    ...
    var pathOptions;
    if (localShape instanceof geometry_1.Path) {
    ...
    if (intersection) {
        ...
        if (localShape instanceof geometry_1.Rectangle) {
            intersection = localShape.getNearestPointToPoint(localRef);
        }
        else if (localShape instanceof geometry_1.Ellipse) {
    ...
    if (options.stroked) {
        cpOffset += util_2.getStrokeWidth(node) / 2;
    }
    return util_2.offset(cp, line.start, cpOffset);
};
