...
var Edge = /** @class */ (function (_super) {
    ...
    Edge.prototype.preprocess = function (metadata, ignoreIdCheck) {
        ...
        if (source != null) {
            if (source instanceof cell_1.Cell) {
                data.source = { cell: source.id };
            }
            else if (isValidId(source)) {
                data.source = { cell: source };
            }
            else if (source instanceof geometry_1.Point) {
        ...
        if (target != null) {
            if (target instanceof cell_1.Cell) {
                data.target = { cell: target.id };
            }
            else if (isValidId(target)) {
                data.target = { cell: target };
            }
            else if (target instanceof geometry_1.Point) {
    ...
    Edge.prototype.getLabels = function () {
        var _this = this;
        return __spreadArrays(this.store.get('labels', [])).map(function (item) {
    ...
    Edge.prototype.getVertices = function () {
        return __spreadArrays(this.store.get('vertices', []));
    };
...
(function (Edge) {
    ...
    function define(config) {
        var constructorName = config.constructorName, others = __rest(config, ["constructorName"]);
