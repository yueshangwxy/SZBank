...
var EdgeView = /** @class */ (function (_super) {
    ...
    EdgeView.prototype.renderStringMarkup = function (markup) {
        ...
        var children = util_1.Dom.createVectors(markup);
    ...
    EdgeView.prototype.parseLabelStringMarkup = function (labelMarkup) {
        var children = util_1.Dom.createVectors(labelMarkup);
    ...
    EdgeView.prototype.normalizeLabelMarkup = function (markup) {
        ...
        if (childNodes.length > 1 || childNodes[0].nodeName.toUpperCase() !== 'G') {
            ...
            // add a `<g/>` container
            vel = util_1.Dom.createVector('g').append(fragment);
        }
        else {
            vel = util_1.Dom.createVector(childNodes[0]);
        }
    ...
    EdgeView.prototype.renderTools = function () {
        ...
        if (markup_1.Markup.isStringMarkup(markup)) {
            ...
            var tool = util_1.Dom.createVector(template());
            ...
            if (this.options.doubleTools) {
                ...
                if (markup_1.Markup.isStringMarkup(doubleToolMarkup)) {
                    ...
                    tool2 = util_1.Dom.createVector(template());
                }
                else {
                    tool2 = tool.clone();
                }
    ...
    EdgeView.prototype.renderVertexMarkers = function () {
        ...
        if (markup_1.Markup.isStringMarkup(markup)) {
            ...
            this.cell.getVertices().forEach(function (vertex, index) {
                $container.append(util_1.Dom.createVector(template_1(__assign({ index: index }, vertex))).node);
            });
        }
        return this;
    };
    EdgeView.prototype.renderArrowheadMarkers = function () {
        ...
        if (markup_1.Markup.isStringMarkup(markup)) {
            var template = util_1.StringExt.template(markup);
            var sourceArrowhead = util_1.Dom.createVector(template({ end: 'source' })).node;
            var targetArrowhead = util_1.Dom.createVector(template({ end: 'target' })).node;
    ...
    EdgeView.prototype.removeRedundantLinearVertices = function (options) {
        ...
        var routePoints = __spreadArrays([this.sourceAnchor], vertices, [this.targetAnchor]);
    ...
    EdgeView.prototype.sendToken = function (token, options, callback) {
        ...
        var vToken = util_1.Dom.createVector(token);
    ...
    EdgeView.prototype.arrowheadDragging = function (target, x, y, data) {
        ...
        if (data.currentTarget !== target) {
            ...
            if (data.currentView) {
                ...
                if (data.currentMagnet && (_a = this.graph.hook).validateConnection.apply(_a, data.getValidateConnectionArgs(data.currentView, data.currentMagnet))) {
        ...
        views.forEach(function (view) {
            ...
            if (view.container.getAttribute('magnet') !== 'false') {
                ...
                if (distance < radius && distance < minDistance) {
                    if (prevMagnet === view.container || (_a = graph.hook).validateConnection.apply(_a, data.getValidateConnectionArgs(view, null))) {
            ...
            view.container.querySelectorAll('[magnet]').forEach(function (magnet) {
                ...
                if (magnet.getAttribute('magnet') !== 'false') {
                    ...
                    if (distance < radius && distance < minDistance) {
                        if (prevMagnet === magnet || (_a = graph.hook).validateConnection.apply(_a, data.getValidateConnectionArgs(view, magnet))) {
                            minDistance = distance;
                            data.closestView = view;
    ...
    EdgeView.prototype.highlightAvailableMagnets = function (data) {
        ...
        var _loop_1 = function (i, ii) {
            ...
            var availableMagnets = magnets.filter(function (magnet) {
                var _a;
                return (_a = graph.hook).validateConnection.apply(_a, data.getValidateConnectionArgs(view, magnet));
            });
    ...
    EdgeView.prototype.stopArrowheadDragging = function (e, x, y) {
        ...
        util_1.FunctionExt.toDeferredBoolean(graph.hook.validateEdge(this.cell, data.terminalType, data.initialTerminal)).then(function (valid) {
            if (valid) {
                _this.finishEmbedding(data);
                _this.notifyConnectionEvent(data, e);
            }
            else {
                // If the changed edge is not allowed, revert to its previous state.
                _this.fallbackConnection(data);
            }
            _this.afterArrowheadDragging(data);
        });
    };
    // #endregion
    // #region drag lable
