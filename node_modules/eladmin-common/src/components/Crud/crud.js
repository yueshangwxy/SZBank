...
function CRUD(options) {
  ...
  const data = {
    ...
    status: {
      ...
      // 标题
      get title() {
        return this.add > CRUD.STATUS.NORMAL ? `新增${crud.title}` : this.edit > CRUD.STATUS.NORMAL ? `编辑${crud.title}` : this.detail > CRUD.STATUS.NORMAL ? `查看${crud.title}` : crud.title
      }
    },
    msg: {
      submit: '提交成功',
  ...
  const methods = {
    ...
    refresh() {
      ...
      return new Promise((resolve, reject) => {
        ...
        initData(crud.url, crud.getQueryParams()).then(data => {
           ...
           console.log('======crud-component>',data)
          if(data == null || data == "") {
            crud.notify("未知错误", CRUD.NOTIFICATION_TYPE.ERROR)
            crud.loading = false
            return
          }
          if(!data.content && data.code != 0) {
            crud.notify(data.msg, CRUD.NOTIFICATION_TYPE.ERROR)
            crud.loading = false
            return
          }
          crud.page.total = data.totalElements == null ? data.data.total : data.totalElements
          crud.data = data.content == null ? data.data.records : data.content
          if (data.content == null && data.data.data) {
            crud.extdata = data.data.data
          }
    ...
    toDetail(data) {
      crud.resetForm(JSON.parse(JSON.stringify(data)))
      if (!(callVmHook(crud, CRUD.HOOK.beforeToDetail, crud.form) && callVmHook(crud, CRUD.HOOK.beforeToDetailCU, crud.form))) {
        return
      }
      crud.status.detail = CRUD.STATUS.PREPARED
      crud.status.flag = 2
      crud.getDataStatus(data.id).detail = CRUD.STATUS.PREPARED
      callVmHook(crud, CRUD.HOOK.afterToDetail, crud.form)
      callVmHook(crud, CRUD.HOOK.afterToDetailCU, crud.form)
    },
    /**
     * 启动删除
     * @param {*} data 数据项
    ...
    cancelCU() {
      ...
      if (detailStatus === CRUD.STATUS.PREPARED) {
        if (!callVmHook(crud, CRUD.HOOK.beforeDetailCancel, crud.form)) {
          return
        }
        crud.status.detail = CRUD.STATUS.NORMAL
        crud.getDataStatus(crud.form.id).detail = CRUD.STATUS.NORMAL
      }
      crud.resetForm()
      if (addStatus === CRUD.STATUS.PREPARED) {
      ...
      if (detailStatus === CRUD.STATUS.PREPARED) {
        callVmHook(crud, CRUD.HOOK.afterDetailCancel, crud.form)
      }
      // 清除表单验证
     ...
     * 通用导出（指定请求方法）
     */
    doExport(method,fileName) {
      crud.downloadLoading = true
      if(!method) method = 'download';
      if(!fileName) fileName = crud.title + '数据';
      download(crud.url + '/' + method, crud.getQueryParams()).then(result => {
    ...
    mounted() {
      ...
      if (this.$refs.table && this.$refs.table.columns) {
        this.$refs.table.columns.forEach(e => {
          ...
          columns[e.property] = {
            label: e.label,
            visible: !((hiddenColumns[e.property] && hiddenColumns[e.property] !== 'visible'))// mod by Suny
          }
        })
      }
...
CRUD.HOOK = {
  ...
  /** 详情 - 之前 */
  beforeToDetail: 'beforeCrudToDetail',
  /** 编辑 - 之后 */
  afterToDetail: 'afterCrudToDetail',
  /** 开始 "编辑" - 之前 */
  beforeToDetailCU: 'beforeCrudToDetailCU',
  /** 开始 "编辑" - 之后 */
  afterToDetailCU: 'afterCrudToDetailCU',
  /** "新建/编辑" 验证 - 之前 */
  ...
  /** 详情取消 - 之前 */
  beforeDetailCancel: 'beforeCrudDetailCancel',
  /** 详情取消 - 之后 */
  afterDetailCancel: 'afterCrudDetailCancel',
  /** 提交 - 之前 */
