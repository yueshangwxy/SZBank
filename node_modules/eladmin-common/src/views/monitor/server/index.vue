  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        <!-- 搜索 -->
        <el-input v-model="query.blurry" clearable size="small" placeholder="输入名称或者服务地址" style="width: 200px;"
    ...
    <el-table ref="table" v-loading="crud.loading" :data="crud.data" style="width: 100%;"
      ...
      <el-table-column v-if="columns.visible('name')" prop="name" label="名称" width="135px"/>
      <el-table-column v-if="columns.visible('address')" prop="address" label="地址" width="80px"/>
      <el-table-column v-if="columns.visible('port')" prop="port" label="端口" width="80px" align="center"/>
      <el-table-column v-if="columns.visible('cpuRate')" :formatter="formatCpuRate" prop="cpuRate" label="CPU使用率"
      ...
      <el-table-column v-if="columns.visible('memTotal')" prop="memTotal" label="物理内存" align="center">
        <template slot-scope="scope">
          ...
          <el-row>
            <el-col :span="24">
              <el-progress :percentage="percentNumber(scope.row.memUsed,scope.row.memTotal)"
                           :status="percentStatus(scope.row.memUsed,scope.row.memTotal)" :show-text="false"/>
            </el-col>
          </el-row>
        </template>
      </el-table-column>
      <el-table-column v-if="columns.visible('diskTotal')" prop="diskTotal" :formatter="formatDisk" label="磁盘使用情况"
                       align="center">
        <template slot-scope="scope">
          <el-row>
          ...
          <el-row>
            <el-col :span="24">
              <el-progress :percentage="percentNumber(scope.row.diskUsed,scope.row.diskTotal)"
                           :status="percentStatus(scope.row.diskUsed,scope.row.diskTotal)" :show-text="false"/>
            </el-col>
          </el-row>
        </template>
      </el-table-column>
      <el-table-column v-if="columns.visible('swapTotal')" prop="swapTotal" label="交换空间" align="center">
        <template slot-scope="scope">
          ...
          <el-row>
            <el-col :span="24">
              <el-progress :percentage="percentNumber(scope.row.swapUsed,scope.row.swapTotal)"
                           :status="percentStatus(scope.row.swapUsed,scope.row.swapTotal)" :show-text="false"/>
            </el-col>
          </el-row>
        </template>
      </el-table-column>
