  ...
  <div class="app-container">
    ...
    >
      <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="80px">
        ...
        <el-form-item v-show="form.type.toString() == '1'" label="数据权限" prop="dataAuthType">
          <el-select v-model="form.dataAuthType" :disabled="form.iframe == 'true'" style="width: 178px;" default-first-option>
            <el-option
    ...
    >
      ...
      <el-table-column min-width="125"
                       ...
                       prop="name"
      />
      <el-table-column v-if="columns.visible('icon')" prop="icon" label="图标" align="center" min-width="60">
        <template slot-scope="scope">
          <svg-icon v-if="scope.row.icon" :icon-class="scope.row.icon"/>
        </template>
      </el-table-column>
      <el-table-column v-if="columns.visible('sort')" prop="sort" align="center" min-width="50" label="排序">
        <!-- <template slot-scope="scope">
           {{ scope.row.sort }}
         </template>-->
      </el-table-column>
      <el-table-column v-if="columns.visible('path')" :show-overflow-tooltip="true" prop="path" min-width="150"
                       label="路由地址"/>
      <el-table-column min-width="100"
                       ...
                       label="权限标识"
      />
      <el-table-column min-width="200" v-if="columns.visible('component')" :show-overflow-tooltip="true"
                       prop="component" label="组件路径"/>
      <el-table-column v-if="columns.visible('iframe')" prop="iframe" label="外链" min-width="50">
        <template slot-scope="scope">
          <span v-if="scope.row.iframe">是</span>
          <span v-else>否</span>
        </template>
      </el-table-column>
      <el-table-column v-if="columns.visible('cache')" prop="cache" label="缓存" min-width="50">
        <template slot-scope="scope">
          <span v-if="scope.row.cache">是</span>
          <span v-else>否</span>
        </template>
      </el-table-column>
      <el-table-column v-if="columns.visible('hidden')" prop="hidden" label="可见" min-width="50">
        <template slot-scope="scope">
          <span v-if="scope.row.hidden">否</span>
          <span v-else>是</span>
        </template>
      </el-table-column>
      <el-table-column v-if="columns.visible('createTime')" prop="createTime" label="创建日期" min-width="150">
        <template slot-scope="scope">
