...
export default {
  ...
  data() {
    return {
      tabList: [
        { label: "已发布的API", name: "publish" },
        { label: "未发布的API", name: "unpublished" },
      ],
      fourthList: [],
      // 分页
      pageSize: 9,
  ...
  methods: {
    ...
    initList(assetsState) {
      ...
      this.$api.getAppointApi(request).then((res) => {
        this.show = true;
        if (res.data) {
    ...
    offline(request) {
      ...
      this.$api.updateApiState(req).then((res) => {
